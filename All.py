...
def summarize_file_with_llm(filepath: Path) -> dict:
    ...
    try:
        ...
        print(f"[DEBUG] LLM raw response for {filepath.name}:
{summary}\n")
        return json.loads(summary)
    except Exception as e:
        print(f"[!] Error summarizing {filepath.name}: {e}")
        return None


# === 전체 처리 ===
def summarize_repo(repo_path: Path):
    ...
    for root, _, files in os.walk(repo_path):
        for file in files:
            ...
            if should_summarize(path):
                ...
                if summary:
                    save_path = SUMMARY_DIR / (path.stem + ".json")
...
if __name__ == "__main__":
    repos = list(BASE_REPO_DIR.glob("*"))
    if not repos:
        print("[X] 분석할 레포가 repos/ 안에 없습니다.")
    elif len(repos) == 1:
        summarize_repo(repos[0])
    else:
        print("[!] repos/ 안에 여러 개의 레포가 있습니다. 수동 선택이 필요합니다.")
        for i, repo in enumerate(repos):
            print(f"{i + 1}. {repo.name}")
        choice = int(input("분석할 레포 번호를 선택하세요: "))
        summarize_repo(repos[choice - 1])
